...
(cd src/wait-assignment && docker build -t $DOCKER_USER/wait-assignment . && docker push $DOCKER_USER/wait-assignment)
...
sed "s/\$DOCKER_USER/$DOCKER_USER/" ./config/wait-assignment-ksvc.yaml.tpl > ./config/wait-assignment-ksvc.yaml